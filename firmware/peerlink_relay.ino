...
void setup() {
  ...
  WiFi.mode(WIFI_AP);
  WiFi.softAP(mySSID.c_str(), "peerlink123"); // Password optional? Prompt said "Open to begin"? Assuming secure for now or open.
  // Prompt: "ESP32 runs as Wi-Fi Access Point". Let's verify password requirement.
  // "Android connects using WifiNetworkSpecifier".
  // If no password, easier. But usually need WPA2. I'll use a default pass "peerlink123".
  
  Serial.println("AP Started: " + mySSID);
  Serial.println("IP: " + WiFi.softAPIP().toString());

  // 2. Setup UDP
  udp.begin(UDP_PORT);
  Serial.println("UDP Listening on 8888");
}
...
void handlePacket(String json, IPAddress senderIP) {
  ...
  if (from != "null" && from.length() > 0) {
    ...
    String name = (type == "presence") ? payload : (clients.count(from) ? clients[from].name : "Unknown");
